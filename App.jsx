...
function ParticipantDashboard({participant,onLogout,toast,tournaments=[],lang:langProp,setLangFn:setLangFnProp}){
  // tData: start dari localStorage, lalu refresh dari Supabase
  const[tData,setTData]=useState(()=>{
    const raw=participant.tournament||{}
    return{...raw,prize:Number(raw.prize||0),entry:Number(raw.entry||0)}
  })
  const t=tData
  ...
  // Fetch fresh tournament data dari Supabase untuk prize pool akurat
  useEffect(()=>{
    const tid=participant.tournamentId
    if(!tid)return
    supabase.from('tournaments').select('*').eq('id',tid).single().then(({data})=>{
      if(data){
        setTData({...data,prize:Number(data.prize||0),entry:Number(data.entry||0)})
        // Update localStorage participant dengan data terbaru
        try{
          const p=JSON.parse(localStorage.getItem('arenagg_participant')||'{}')
          p.tournament={...data,prize:Number(data.prize||0),entry:Number(data.entry||0)}
          localStorage.setItem('arenagg_participant',JSON.stringify(p))
        }catch(e){}
      }
    })
  ...
  const TABS=[
    {id:'home',icon:'🏠',label:i.pd_home||'Beranda'},
  ...
  return <div style={{minHeight:'100vh',background:'var(--bg)'}}>
    ...
    <div style={{background:'rgba(5,5,8,0.97)',borderBottom:'1px solid var(--border)',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:50,backdropFilter:'blur(10px)'}}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div style={{fontFamily:'var(--fh)',fontSize:10,color:'var(--orange)',letterSpacing:1}}>{i.pd_portal||'PORTAL PESERTA'}</div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        <div style={{fontFamily:'var(--fh)',fontSize:10,fontWeight:700,color:'var(--cyan)'}}>{participant.name}</div>
        <LangSelector lang={lang} setLangFn={setLangFn}/>
        <button onClick={onLogout} style={{background:'none',border:'1px solid var(--border)',borderRadius:4,padding:'3px 9px',color:'var(--muted)',cursor:'pointer',fontSize:9,fontFamily:'var(--fm)'}}>{i.pd_logout||'Keluar'}</button>
      </div>
    </div>
    ...
    <div style={{maxWidth:600,margin:'0 auto',padding:'16px 14px'}}>
      ...
      {activeTab==='home'&&<div className="animate-in pd-content" style={{padding:'0 4px'}}>
        ...
        <div style={{background:'linear-gradient(135deg,rgba(255,107,0,0.1),rgba(0,229,255,0.06))',border:'1px solid rgba(255,107,0,0.25)',borderRadius:14,padding:'18px',marginBottom:14,display:'flex',gap:14,alignItems:'center'}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
              <span className={`tag tag-${participant.paid?'active':'pending'}`} style={{fontSize:8}}>{participant.paid?(i.pd_lunas||'✓ LUNAS'):(i.pd_blm_bayar||'⏳ BELUM BAYAR')}</span>
        ...
        {t&&<div className="card" style={{marginBottom:14}}>
          <div style={{fontFamily:'var(--fh)',fontSize:10,color:'var(--cyan)',letterSpacing:1,marginBottom:12}}>{i.pd_tourn_info||'🏆 INFO TURNAMEN'}</div>
